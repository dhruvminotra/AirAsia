...
/**
 * Searcher for AirAsia X — IATA carrier {@code D7} — on its Navitaire New Skies
 * instance. Mirrors the prod {@code NavitaireSearcher} pattern.
 */
@Component
public class AirAsiaXSearcher extends AbstractFlightSearcher {

    private final MockNavitaireEngine engine;
    private final ProviderSettings settings;

    public AirAsiaXSearcher(MockNavitaireEngine engine, ProviderProperties properties) {
        this.engine = engine;
        this.settings = properties.getAirAsiaX();
    }
